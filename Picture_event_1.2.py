...
class MyWindow(QMainWindow):
    ...
    def initDevice(self, flag, num):
        ...
        self.l7 = MyLabel()
        if number % 2 == 0:
            self.l7.setPixmap(fjl.scaled(xft_dl.width() * 5 * ratio, xft_dl.height() * 3 * ratio))
        else:
            self.l7.setPixmap(fjl.scaled(xft_dl.width() * 7 * ratio, xft_dl.height() * 3 * ratio))
        self.l7.setObjectName('分解炉')
        self.l7.changeindex.connect(self.change_pic)

        self.l9 = MyLabel()
        self.l9.setPixmap(yao.scaled(xft_dl.width() * 9 * ratio, yao.height() * ratio))
        ...
        if number % 2 == 0:
            lay.addWidget(self.l7, number - 2, 2, fjl_row, fjl_col)  # 分解炉
            lay.addWidget(self.xft[number], number - 1, 2)
            lay.addWidget(self.fjl_yao, number - 2 + fjl_row, (fjl_col - 1) / 2 + 2)  # 连接件
            lay.addWidget(self.l9, number - 2 + fjl_row, (fjl_col - 1) / 2 + 3, 2, 9)  # 窑
            lay.addWidget(self.l10, number - 2 + fjl_row, 16, 2, 3)  # 篦冷机
        else:
            lay.addWidget(self.l7, number - 2, 1, fjl_row, fjl_col + 2)  # 分解炉
    ...
    def initPic(self):
        ...
        self.l7 = MyLabel()
        self.l7.setPixmap(fjl.scaled(xft_dl.width() * 6.9 * ratio, xft_dl.width() * 3 * ratio))
        self.l7.setObjectName('分解炉')
        self.l7.changeindex.connect(self.change_pic)
        ...
        '''self.l11 = MyLabel()
        self.l11.setPixmap(xft_zuo.scaled(xft_zuo.width() * ratio, xft_zuo.height() * ratio))
        self.l11.setObjectName('一级筒B')
        self.l11.changeindex.connect(self.change_pic)

        self.l12 = MyLabel()
        self.l12.setPixmap(xft_dl.scaled(xft_dl.width() * ratio, xft_dl.height() * ratio))
        self.l12.setObjectName('一级筒B')
        self.l12.changeindex.connect(self.change_pic)

        self.l13 = MyLabel()
        self.l13.setPixmap(xft_you.scaled(xft_you.width() * ratio, xft_you.height() * ratio))
        self.l13.setObjectName('二级筒B')
        self.l13.changeindex.connect(self.change_pic)

        self.l14 = MyLabel()
        self.l14.setPixmap(xft_zuo.scaled(xft_zuo.width() * ratio, xft_zuo.height() * ratio))
        self.l14.setObjectName('三级筒B')
        self.l14.changeindex.connect(self.change_pic)

        self.l15 = MyLabel()
        self.l15.setPixmap(xft_you.scaled(xft_you.width() * ratio, xft_you.height() * ratio))
        self.l15.setObjectName('四级筒B')
        self.l15.changeindex.connect(self.change_pic)

        self.l16 = MyLabel()
        self.l16.setPixmap(xft_dl.scaled(xft_dl.width() * ratio, xft_dl.height() * ratio))
